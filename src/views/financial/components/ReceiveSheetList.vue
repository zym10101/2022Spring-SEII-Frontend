    ...
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        ...
        <div v-if="showAll[index]" style="margin-top: 15px">
          <div style="margin-bottom: 15px"><strong>详细收款单:</strong></div>
          <el-table
...
<script>
import {receiveSheetApproval} from "@/network/Financial";

export default {
  name: "receiveSheetList",
  props: {
    list: Array,
    type: Number
  },
  ...
  methods: {
    ...
    approval(id) {
      ...
      if (this.type === 1) {
        receiveSheetApproval(config).then(res => {
          this.$emit("refresh");
          this.$message({
            message: "操作成功!",
            type: "success"
